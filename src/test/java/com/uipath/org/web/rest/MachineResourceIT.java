...
class MachineResourceIT {
    ...
    @Test
    @Transactional
    void getAllMachinesByUrobotIsEqualToSomething() throws Exception {
        // Initialize the database
        machineRepository.saveAndFlush(machine);
        Urobot urobot = UrobotResourceIT.createEntity(em);
        em.persist(urobot);
        em.flush();
        machine.setUrobot(urobot);
        machineRepository.saveAndFlush(machine);
        Long urobotId = urobot.getId();

        // Get all the machineList where urobot equals to urobotId
        defaultMachineShouldBeFound("urobotId.equals=" + urobotId);

        // Get all the machineList where urobot equals to (urobotId + 1)
        defaultMachineShouldNotBeFound("urobotId.equals=" + (urobotId + 1));
    }
