...
class UpackageResourceIT {
    ...
    @Test
    @Transactional
    void getAllUpackagesByProcessIsEqualToSomething() throws Exception {
        // Initialize the database
        upackageRepository.saveAndFlush(upackage);
        Process process = ProcessResourceIT.createEntity(em);
        em.persist(process);
        em.flush();
        upackage.setProcess(process);
        upackageRepository.saveAndFlush(upackage);
        Long processId = process.getId();

        // Get all the upackageList where process equals to processId
        defaultUpackageShouldBeFound("processId.equals=" + processId);

        // Get all the upackageList where process equals to (processId + 1)
        defaultUpackageShouldNotBeFound("processId.equals=" + (processId + 1));
    }
